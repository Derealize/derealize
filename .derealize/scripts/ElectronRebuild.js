...
if (depKeys.length > 0 && fs.existsSync(nodeModulesPath)) {
  const onlyModules = depKeys.join(',')
  const electronRebuildCmd = `../node_modules/.bin/electron-rebuild --parallel --force --only ${onlyModules} --types prod,dev,optional --module-dir .`
