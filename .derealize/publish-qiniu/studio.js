...
const fileName = `Derealize-studio-${process.env.npm_package_version}.${isDarwin ? 'dmg' : 'exe'}`
const putPolicy = new qiniu.rs.PutPolicy({ scope: 'derealize' })
const uploadToken = putPolicy.uploadToken(mac)

const filePath = path.join(__dirname, `../../release-studio/${fileName}`)
...
// const fileNameStatic = `Derealize-studio.${isDarwin ? 'dmg' : 'exe'}`
