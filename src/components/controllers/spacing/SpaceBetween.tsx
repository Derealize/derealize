...
const SpaceBetween: React.FC<Props> = ({ already }: Props): JSX.Element => {
  ...
  const values = useStoreState<Array<string>>((state) => state.spacing.spaceBetweenValues)
  const propertys = useStoreState<Array<Property>>((state) => state.spacing.spaceBetweenPropertys)
