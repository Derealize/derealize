...
const closest = (value, array) => {
  ...
  array.forEach((v, i) => {
    if (Math.abs(value - v) < infinity) {
      infinity = Math.abs(value - v)
      closestIndex = i
    }
  })
...
class ChromeTabs {
  ...
  addTab(tabProperties, { animate = true, background = false } = {}) {
    ...
    const properties = { ...defaultTapProperties, ...tabProperties }
    this.tabContentEl.appendChild(tabEl)
    this.setTabCloseEventListener(tabEl)
    updateTab(tabEl, properties)
